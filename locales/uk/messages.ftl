...
# Підказки для ІІ
prompt-analyze-document = Проаналізуйте текст із цього документа '{ $filename }' українською мовою:
prompt-describe-image-default = Опишіть це зображення українською мовою.
response-text-truncated-for-ai = [... Текст було скорочено перед відправкою ІІ через обмеження довжини ...]
...
error-gemini-api-key-invalid = 🔑 Помилка: недійсний або відкликаний API-ключ Gemini. Перевірте конфігурацію.
error-gemini-service-unavailable = ☁️ Сервіс Gemini тимчасово недоступний. Спробуйте пізніше.
error-gemini-unknown = ❓ Виникла невідома помилка API Gemini ({ $type }). Зверніться до адміністратора.
error-image-connection-error = 🌐 Не вдалося під’єднатися до сервісу генерації зображень. Перевірте мережу або спробуйте пізніше.
error-telegram-download = 😔 Не вдалося завантажити файл з Telegram. Будь ласка, надішліть його ще раз.
error-telegram-upload = 😔 Не вдалося завантажити файл/фото до Telegram.
error-db-save = 💾 Не вдалося зберегти дані до бази даних. Налаштування або історія могли не оновитися.
error-telegram-network = 🌐 Помилка мережі під’єднання до Telegram. Перевірте з’єднання або спробуйте пізніше.
error-message-deleted = 🤷 Схоже, повідомлення, на яке я відповідав, було видалено.
delete-not-found = Здається, з вашим обліковим записом не було пов’язано даних для видалення.