...
start-welcome = Привіт, { $user_name }! Я бот зі штучним інтелектом на базі Gemini. Надішліть мені текст або зображення (з підписом або без), і я спробую відповісти. Використовуйте /help для перегляду команд.
...
help-text =
    ...
    - Надішліть зображення. Ви можете додати підпис до зображення, щоб поставити конкретне запитання про нього (наприклад, "Що незвичайного на цій картинці?"). Якщо підпису немає, я просто опишу його.
...
error-gemini-request = Сталася помилка під час звернення до Gemini: { $error }
error-image-analysis-request = Сталася помилка під час аналізу зображення: { $error }