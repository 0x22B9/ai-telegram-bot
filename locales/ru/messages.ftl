...
# Подсказки для ИИ
prompt-analyze-document = Проанализируйте текст из этого документа '{ $filename }' на русском языке:
prompt-describe-image-default = Опишите это изображение на русском языке.
response-text-truncated-for-ai = [... Текст был сокращён перед отправкой ИИ из-за ограничений длины ...]
...
error-download-image = 😔 Не удалось загрузить ваше изображение. Попробуйте еще раз.
error-image-analysis-failed = ❌ Не удалось проанализировать изображение с помощью ИИ. Попробуйте позже.
error-image-analysis-unknown = ❓ Произошла неизвестная ошибка при анализе изображения.
error-gemini-api-key-invalid = 🔑 Ошибка: недействительный или отозванный API-ключ Gemini. Проверьте конфигурацию.
error-gemini-service-unavailable = ☁️ Сервис Gemini временно недоступен. Попробуйте позже.
error-gemini-unknown = ❓ Произошла неизвестная ошибка API Gemini ({ $type }). Обратитесь к администратору.
error-image-connection-error = 🌐 Не удалось подключиться к сервису генерации изображений. Проверьте сеть или попробуйте позже.
error-telegram-download = 😔 Не удалось скачать файл из Telegram. Пожалуйста, отправьте его ещё раз.
error-telegram-upload = 😔 Не удалось загрузить файл/фото в Telegram.
error-db-save = 💾 Не удалось сохранить данные в базе данных. Настройки или история могли не обновиться.
error-telegram-network = 🌐 Ошибка сети при подключении к Telegram. Проверьте соединение или попробуйте позже.
error-message-deleted = 🤷 Похоже, сообщение, на которое я отвечал, было удалено.
delete-not-found = Похоже, с вашим аккаунтом не было связано никаких данных для удаления.