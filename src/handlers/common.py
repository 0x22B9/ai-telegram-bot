...
from src.localization import LOCALIZATIONS, SUPPORTED_LOCALES, get_localizer
...
async def send_language_selection_message(
...
async def handle_start(
    ...
    if current_lang_code and current_lang_code in SUPPORTED_LOCALES:
        welcome_message = localizer.format_value(
            "start-welcome", args={"user_name": user_name}
        )
...
async def handle_language_selection(
    ...
    welcome_message = new_localizer.format_value(
        "language-chosen", args={"user_name": user_name}
    )

    try:
        keyboard = get_main_keyboard(new_localizer)
        await callback_query.bot.send_message(
            chat_id=user_id, text=welcome_message, reply_markup=keyboard
        )
        if callback_query.message:
            try:
                ...
                if "message to delete not found" in str(e_del).lower():
                    logger.warning(
                        f"Lang select: Message {callback_query.message.message_id} already deleted."
                    )
                else:
                    logger.error(
                        f"Lang select: Error deleting message {callback_query.message.message_id}: {e_del}"
                    )
            except TelegramNetworkError as e_net:
                logger.error(f"Lang select: Network error deleting message: {e_net}")
            except Exception as e_unexp:
                logger.error(
                    f"Lang select: Unexpected error deleting message: {e_unexp}",
                    exc_info=True,
                )

    except (TelegramNetworkError, TelegramBadRequest) as e_send:
        logger.error(
            f"Lang select: Error sending welcome message for user {user_id}: {e_send}"
        )
    except Exception as e_unexp_send:
        logger.error(
            f"Lang select: Unexpected error sending welcome message for user {user_id}: {e_unexp_send}",
            exc_info=True,
        )
...
async def handle_model_selection(
    ...
    response_text = current_localizer.format_value(
        "model-chosen", args={"model_name": selected_model}
    )
    ...
    try:
        ...
        if callback_query.message:
            try:
                ...
                if "message to delete not found" in str(e_del).lower():
                    logger.warning(
                        f"Model select: Message {callback_query.message.message_id} already deleted."
                    )
                else:
                    logger.error(
                        f"Model select: Error deleting message {callback_query.message.message_id}: {e_del}"
                    )
            except TelegramNetworkError as e_net:
                logger.error(f"Model select: Network error deleting message: {e_net}")
            except Exception as e_unexp:
                logger.error(
                    f"Model select: Unexpected error deleting message: {e_unexp}",
                    exc_info=True,
                )

    except (TelegramNetworkError, TelegramBadRequest) as e_send:
        logger.error(
            f"Model select: Error sending confirmation for user {user_id}: {e_send}"
        )
    except Exception as e_unexp_send:
        logger.error(
            f"Model select: Unexpected error sending confirmation for user {user_id}: {e_unexp_send}",
            exc_info=True,
        )
...
async def handle_new_chat(message: types.Message, localizer: FluentLocalization):
    ...
    try:
        ...
        if success:
            ...
            logger.error(
                f"Failed to clear history (clear_history returned False) for user {user_id}."
            )
...
async def handle_help(message: types.Message, localizer: FluentLocalization):
    ...
    try:
        ...
        logger.error(
            f"Help: Could not send help message to user {message.from_user.id}: {e_send}"
        )
