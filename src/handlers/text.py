...
from aiogram.exceptions import TelegramBadRequest, TelegramNetworkError, TelegramRetryAfter # Добавили исключения
...
from src.services.gemini import (
    ...
    GEMINI_QUOTA_ERROR, GEMINI_API_KEY_INVALID, GEMINI_SERVICE_UNAVAILABLE,
    GEMINI_UNKNOWN_API_ERROR
)
from src.services.errors import (
    format_error_message, DATABASE_SAVE_ERROR, TELEGRAM_NETWORK_ERROR,
...
async def send_typing_periodically(bot: Bot, chat_id: int):
    ...
    try:
        ...
        logger.warning(f"Error in send_typing_periodically for chat {chat_id}: {e}", exc_info=False)
...
async def _process_text_input(
    ...
    try:
        ...
        if response_text and not error_code:
            ...
            logger.error(f"Unexpected result from Gemini ({selected_model}) for user_id={user_id}: no text and no error code.")
...
async def handle_text_message(message: types.Message, state: FSMContext, bot: Bot, localizer: FluentLocalization):
    ...
    try:
        ...
        logger.exception(f"Critical error in handler logic for user_id={user_id} while processing text: {e}")
        final_response, _ = format_error_message(None, localizer)
        save_needed = False
        failed_prompt = None
    finally:
    ...
    try:
        ...
        logger.warning(f"Flood control exceeded for user {user_id}: retry after {e.retry_after}s")
        await asyncio.sleep(e.retry_after)
        try:
             await thinking_message.edit_text(final_response, reply_markup=reply_markup)
        except Exception as retry_e:
             logger.error(f"Failed to edit message even after RetryAfter for user {user_id}: {retry_e}")
             save_needed = False
    except TelegramBadRequest as e:
        if "message is not modified" in str(e):
             logger.debug(f"Message {thinking_message.message_id} is not modified.")
        elif "message to edit not found" in str(e).lower():
             logger.warning(f"Message {thinking_message.message_id} to edit not found for user {user_id}. Sending new message.")
             try:
                  main_kbd = get_main_keyboard(localizer)
                  await message.answer(final_response, reply_markup=reply_markup or main_kbd)
             except Exception as send_e:
                  logger.error(f"Failed to send new message after edit failed for user {user_id}: {send_e}")
                  save_needed = False
            ...
            try:
                ...
                logger.error(f"Failed to send plain text for user_id={user_id}: {fallback_e}", exc_info=True)
                await thinking_message.edit_text(localizer.format_value('error-display'))
        else:
            logger.error(f"Unexpected TelegramBadRequest for user_id={user_id}: {e}", exc_info=True)
            await thinking_message.edit_text(localizer.format_value('error-telegram-send'))
            save_needed = False
    except TelegramNetworkError as e:
         logger.error(f"Network error while editing message for user {user_id}: {e}")
         await thinking_message.edit_text(localizer.format_value('error-telegram-network'))
         save_needed = False
    except Exception as e:
    ...
    if save_needed and updated_history is not None:
        try:
            ...
            logger.exception(f"Failed to save history for user_id={user_id} to DB: {db_save_e}")
            await message.answer(localizer.format_value('error-db-save'))
    elif save_needed and updated_history is None:
         logger.error(f"Flag save_needed is True, but updated_history is None for user_id={user_id}!")
...
async def handle_retry_request(callback: types.CallbackQuery, state: FSMContext, bot: Bot, localizer: FluentLocalization):
    ...
    try:
        ...
        logger.warning(f"Retry Handler: Could not answer callback for user {user_id}: {e_ans}")

    if not original_prompt:
        logger.warning(f"Retry Handler: Text for retry not found for user_id={user_id}.")
        if callback.message:
            try:
                await callback.message.edit_text(
                    localizer.format_value('error-retry-not-found'),
                    reply_markup=None
                )
            except Exception as e_edit:
                logger.error(f"Retry Handler: Error editing 'retry not found' message for user_id={user_id}: {e_edit}")
        return
    ...
    if status_message:
        try:
            ...
            logger.warning(f"Retry Handler: Could not update status before retrying for user_id={user_id}: {e_edit_status}")
    else:
        logger.warning(f"Retry Handler: No message found in callback for user {user_id}. Cannot edit status.")
    ...
    try:
        ...
        logger.exception(f"Retry Handler: Critical error in handler logic for user_id={user_id}: {e}")
        final_response, _ = format_error_message(None, localizer)
        save_needed = False
        failed_prompt = None
    finally:
    ...
    if status_message:
        try:
            ...
            logger.warning(f"Retry Handler: Flood control for user {user_id}: retry after {e.retry_after}s")
            await asyncio.sleep(e.retry_after)
            try:
                 await status_message.edit_text(final_response, reply_markup=reply_markup)
                 message_sent_or_edited = True
            except Exception as retry_e:
                 logger.error(f"Retry Handler: Failed edit after RetryAfter for user {user_id}: {retry_e}")
        except TelegramBadRequest as e:
            if "message is not modified" in str(e):
                 logger.debug(f"Retry Handler: Message {status_message.message_id} not modified.")
                 message_sent_or_edited = True
            elif "message to edit not found" in str(e).lower():
                 logger.warning(f"Retry Handler: Message {status_message.message_id} not found for user {user_id}. Sending new.")
                 try:
                      main_kbd = get_main_keyboard(localizer)
                      await bot.send_message(chat_id, final_response, reply_markup=reply_markup or main_kbd)
                      message_sent_or_edited = True
                 except Exception as send_e:
                      logger.error(f"Retry Handler: Failed send new message for user {user_id}: {send_e}")
            elif "can't parse entities" in str(e):
                logger.warning(f"Retry Handler: Parse error for user_id={user_id}. Plain text. Error: {e}")
                try:
                    await status_message.edit_text(final_response, parse_mode=None, reply_markup=reply_markup)
                    message_sent_or_edited = True
                except Exception as fallback_e:
                    logger.error(f"Retry Handler: Failed send plain text for user_id={user_id}: {fallback_e}", exc_info=True)
                    try: await status_message.edit_text(localizer.format_value('error-display'))
                    except Exception: pass
            else:
                logger.error(f"Retry Handler: Unexpected TelegramBadRequest for user_id={user_id}: {e}", exc_info=True)
                try: await status_message.edit_text(localizer.format_value('error-telegram-send'))
                except Exception: pass
        except TelegramNetworkError as e:
             logger.error(f"Retry Handler: Network error editing message for user {user_id}: {e}")
             try:
                  err_msg_net, _ = format_error_message(TELEGRAM_NETWORK_ERROR, localizer)
                  await status_message.edit_text(err_msg_net)
             except Exception: pass
        except Exception as e:
            logger.exception(f"Retry Handler: Failed edit final response for user_id={user_id}: {e}")
            try:
                await status_message.edit_text(localizer.format_value('error-general'))
            except Exception: pass
    else:
         logger.warning(f"Retry Handler: status_message was None for user {user_id}, sending new.")
         try:
              main_kbd = get_main_keyboard(localizer)
              await bot.send_message(chat_id, final_response, reply_markup=reply_markup or main_kbd)
              message_sent_or_edited = True
         except Exception as e_send:
              logger.error(f"Retry Handler: Failed to send fallback message for user {user_id}: {e_send}")

    if save_needed and updated_history is not None and message_sent_or_edited:
        try:
            await save_history(user_id, updated_history)
        except Exception as db_save_e:
            logger.exception(f"Retry Handler: Failed to save history for user_id={user_id} to DB: {db_save_e}")
            try:
                 ...
                 logger.error(f"Retry Handler: Failed send DB save error message to user {user_id}: {db_err_send_e}")
    elif save_needed and updated_history is None:
        logger.error(f"Retry Handler: save_needed is True, but updated_history is None for user_id={user_id}!")