...
from src.services.errors import (
    format_error_message, TELEGRAM_DOWNLOAD_ERROR, TELEGRAM_NETWORK_ERROR,
    DATABASE_SAVE_ERROR, TELEGRAM_MESSAGE_DELETED_ERROR,
    PARSING_LIB_MISSING
)
from src.db import save_history, get_history
...
async def handle_document_message(message: types.Message, state: FSMContext, bot: Bot, localizer: FluentLocalization):
    ...
    if document.file_size > MAX_DOCUMENT_SIZE_BYTES:
        ...
        error_code_with_arg = f"DOC_TOO_LARGE{doc_parser.FTL_ARGS_SEPARATOR}limit_mb={limit_mb_str}"
        error_text = localizer.format_value("error-doc-too-large", args={"limit_mb": limit_mb_str})
        await message.reply(error_text)
        return

    logger.info(f"Got document from user_id={user_id}: mime_type={mime_type}, filename={document.file_name}, size={document.file_size}")

    processing_doc_text = localizer.format_value("processing-document", args={"filename": document.file_name or "document"})
    ...
    try:
        ...
        if extracted_text and parsing_error_code == doc_parser.PARSING_SUCCESS:
            ...
            processing_text_status = localizer.format_value("processing-extracted-text", args={"filename": document.file_name or "document"})
            try:
                await status_message.edit_text(processing_text_status)
            except Exception as e_edit_status:
                 logger.warning(f"Could not edit status message for document processing: {e_edit_status}")
            ...
            if len(user_input_for_gemini) > MAX_PROMPT_LENGTH_FOR_AI:
                logger.warning(f"Extracted text for user {user_id} is too long ({len(user_input_for_gemini)}), truncating for AI.")
                truncation_marker = localizer.format_value(
                    "response-text-truncated-for-ai"
                )
                allowed_text_length = MAX_PROMPT_LENGTH_FOR_AI - len(prompt_intro) - len(truncation_marker) - 5
                if allowed_text_length < 0: allowed_text_length = 0
            ...
            save_needed = updated_history is not None and failed_prompt_for_retry is None

        else:
            logger.warning(f"Text extraction error from document {document.file_name} for user_id={user_id}: {parsing_error_code}")
            ...
            final_response, _ = format_error_message(error_code_to_format, localizer, 'error-doc-parsing-unknown')
            save_needed = False
         ...
         if isinstance(e, (TelegramNetworkError, TelegramBadRequest)):
              logger.error(f"Failed to download document {document.file_id} for user {user_id}: {e}", exc_info=True)
              final_response, _ = format_error_message(TELEGRAM_DOWNLOAD_ERROR, localizer)
         elif isinstance(e, ValueError):
              logger.error(f"Downloaded document {document.file_id} for user {user_id} was empty.")
              final_response, _ = format_error_message(TELEGRAM_DOWNLOAD_ERROR, localizer)
         else:
              logger.exception(f"Unexpected error processing document for user_id={user_id}")
    ...
    try:
        ...
        if len(final_response) > MAX_RESPONSE_LEN:
            ...
            final_response = final_response[:MAX_RESPONSE_LEN - len(cutoff_msg) - 5] + f"\n\n{cutoff_msg}"

        await status_message.edit_text(final_response, reply_markup=reply_markup)
        message_sent_or_edited = True
    except TelegramRetryAfter as e:
        logger.warning(f"Document Handler: Flood control for user {user_id}: retry after {e.retry_after}s")
        await asyncio.sleep(e.retry_after)
        try:
             await status_message.edit_text(final_response, reply_markup=reply_markup)
             message_sent_or_edited = True
        except Exception as retry_e:
             logger.error(f"Document Handler: Failed edit after RetryAfter for user {user_id}: {retry_e}")
    except TelegramBadRequest as e:
        if "message is not modified" in str(e):
             logger.debug(f"Document Handler: Message {status_message.message_id} not modified.")
             message_sent_or_edited = True
        elif "message to edit not found" in str(e).lower():
             logger.warning(f"Document Handler: Message {status_message.message_id} not found for user {user_id}. Sending new.")
             try:
                  main_kbd = get_main_keyboard(localizer)
                  await message.answer(final_response, reply_markup=reply_markup or main_kbd)
                  message_sent_or_edited = True
             except Exception as send_e:
                  logger.error(f"Document Handler: Failed send new message for user {user_id}: {send_e}")
        elif "can't parse entities" in str(e) or "nested entities" in str(e):
            logger.warning(f"Document Handler: Parse error for user_id={user_id}. Plain text. Error: {e}")
            try:
                await status_message.edit_text(final_response, parse_mode=None, reply_markup=reply_markup)
                message_sent_or_edited = True
            except Exception as fallback_e:
                logger.error(f"Document Handler: Failed send plain text for user_id={user_id}: {fallback_e}", exc_info=True)
                try: await status_message.edit_text(localizer.format_value('error-display'))
                except Exception: pass
        else:
            logger.error(f"Document Handler: Unexpected TelegramBadRequest for user_id={user_id}: {e}", exc_info=True)
            try: await status_message.edit_text(localizer.format_value('error-telegram-send'))
            except Exception: pass
    except TelegramNetworkError as e:
         logger.error(f"Document Handler: Network error editing message for user {user_id}: {e}")
         try:
              err_msg_net, _ = format_error_message(TELEGRAM_NETWORK_ERROR, localizer)
              await status_message.edit_text(err_msg_net)
         except Exception: pass
    except Exception as e:
        logger.exception(f"Document Handler: Failed edit final response for user_id={user_id}: {e}")
        try:
    ...
    if save_needed and updated_history is not None and message_sent_or_edited:
        try:
            ...
            logger.exception(f"Document Handler: Failed to save history for user_id={user_id} to DB: {db_save_e}")
            try:
                 ...
                 logger.error(f"Document Handler: Failed send DB save error message to user {user_id}: {db_err_send_e}")
    elif save_needed and updated_history is None:
         logger.error(f"Document Handler: save_needed is True, but updated_history is None for user_id={user_id}!")