...
from src.services.errors import format_error_message, TELEGRAM_MESSAGE_DELETED_ERROR, TELEGRAM_NETWORK_ERROR
...
async def handle_delete_my_data_command(message: types.Message, localizer: FluentLocalization):
    ...
    try:
        await message.answer(
         ...
         logger.error(f"DeleteData: Could not send confirmation to user {message.from_user.id}: {e_send}")
...
async def handle_delete_confirmation_callback(callback: types.CallbackQuery, localizer: FluentLocalization):
    ...
    try:
        await callback.answer()
         ...
         logger.warning(f"DeleteData: Could not answer callback for user {user_id}: {e_ans}")
    ...
    if action == "yes":
        ...
        try:
             ...
             logger.exception(f"DeleteData: Error deleting data for user {user_id} from DB: {e_db}")
        ...
        logger.error(f"Unknown action in delete callback: {callback.data} from user_id={user_id}")
    ...
    if message_to_edit:
        try:
            ...
            logger.warning(f"DeleteData: Flood control editing message for user {user_id}: retry after {e.retry_after}s")
            await asyncio.sleep(e.retry_after)
            try: await message_to_edit.edit_text(result_text, reply_markup=None)
            except Exception: pass
        except TelegramBadRequest as e:
            if "message to edit not found" in str(e).lower():
                 logger.warning(f"DeleteData: Message {message_to_edit.message_id} not found. Sending new.")
                 try:
                      await callback.bot.send_message(user_id, result_text)
                 except Exception as send_e:
                      logger.error(f"DeleteData: Failed send new message for user {user_id}: {send_e}")
            else:
                 logger.error(f"DeleteData: Error editing message for user {user_id}: {e}")
        except TelegramNetworkError as e_net:
             logger.error(f"DeleteData: Network error editing message for user {user_id}: {e_net}")
        except Exception as e_unexp:
             logger.error(f"DeleteData: Unexpected error editing message for user {user_id}: {e_unexp}", exc_info=True)
    else:
         logger.warning(f"DeleteData: No message found in callback query for user {user_id}")
         try: await callback.bot.send_message(user_id, result_text)
         except Exception as send_e: logger.error(f"DeleteData: Failed send message directly for user {user_id}: {send_e}")