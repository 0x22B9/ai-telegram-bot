...
from src.services.errors import format_error_message, TELEGRAM_UPLOAD_ERROR, TELEGRAM_NETWORK_ERROR
...
async def send_upload_photo_periodically(bot: Bot, chat_id: int):
    ...
    try:
        ...
        logger.warning(f"Error in send_upload_photo_periodically for chat {chat_id}: {e}", exc_info=False)
...
async def handle_image_prompt(message: types.Message, state: FSMContext, bot: Bot, localizer: FluentLocalization):
    ...
    try:
        ...
        if image_bytes and error_code == img_gen_service.IMAGE_GEN_SUCCESS:
            ...
            try:
                await message.answer_photo(photo=photo_input, caption=caption_text, reply_markup=main_kbd)
                logger.info(f"Generated image sent to user_id={user_id}")
                try:
                    await status_message.delete()
                except Exception as e_del:
                     logger.warning(f"Could not delete status message {status_message.message_id}: {e_del}")

            except TelegramRetryAfter as e:
                logger.warning(f"Image Gen: Flood control sending photo for user {user_id}: retry after {e.retry_after}s")
                await asyncio.sleep(e.retry_after)
                try:
                     await message.answer_photo(photo=photo_input, caption=caption_text, reply_markup=main_kbd)
                     logger.info(f"Generated image sent to user_id={user_id} after retry.")
                     try: await status_message.delete()
                     except Exception: pass
                except Exception as retry_e:
                     logger.error(f"Image Gen: Failed send photo after RetryAfter for user {user_id}: {retry_e}")
                     err_msg_upload, _ = format_error_message(TELEGRAM_UPLOAD_ERROR, localizer)
                     await status_message.edit_text(err_msg_upload)
            except (TelegramBadRequest, TelegramNetworkError) as e:
                logger.error(f"Image Gen: Failed to send photo for user {user_id}: {e}", exc_info=True)
                error_key = TELEGRAM_NETWORK_ERROR if isinstance(e, TelegramNetworkError) else TELEGRAM_UPLOAD_ERROR
                err_msg_upload, _ = format_error_message(error_key, localizer)
                try: await status_message.edit_text(err_msg_upload)
                except Exception: pass
            except Exception as e_send:
                logger.exception(f"Image Gen: Unexpected error sending photo for user {user_id}: {e_send}")
                err_msg_upload, _ = format_error_message(TELEGRAM_UPLOAD_ERROR, localizer)
                try: await status_message.edit_text(err_msg_upload)
                except Exception: pass

        else:
            logger.warning(f"Image generation failed for user_id={user_id}: {error_code}")
            final_response_text, _ = format_error_message(error_code, localizer, 'error-image-unknown')
            try:
                 await status_message.edit_text(final_response_text)
            except Exception as e_edit_err:
                  logger.error(f"Image Gen: Could not edit status message with error: {e_edit_err}")

    except Exception as e:
        logger.exception(f"Image Gen: Unexpected error in handler for user {user_id}: {e}")
