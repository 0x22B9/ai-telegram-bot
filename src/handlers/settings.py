...
async def build_settings_keyboard(
    user_id: int, localizer: FluentLocalization
    ...
    temp_button_text = localizer.format_value(
        "settings-button-temperature", args={"value": temp_name}
    )
    builder.button(text=temp_button_text, callback_data="settings:set:temperature")
    tokens_button_text = localizer.format_value(
        "settings-button-max-tokens", args={"value": tokens_name}
    )
...
async def _edit_settings_message(
    ...
    try:
        ...
        logger.warning(
            f"Settings: Flood control editing settings for user {message.chat.id}: retry after {e.retry_after}s"
        )
        ...
        logger.error(
            f"Settings: Network error editing message {message.message_id}: {e_net}"
        )
    except Exception as e_unexp:
        logger.error(
            f"Settings: Unexpected error editing message {message.message_id}: {e_unexp}",
            exc_info=True,
        )
...
async def handle_settings_command(
    ...
    try:
        ...
        await message.answer(
            settings_text, reply_markup=keyboard.as_markup(), parse_mode=None
        )
    except Exception as e:
        logger.error(
            f"Settings: Error sending settings message for user {user_id}: {e}",
            exc_info=True,
        )
...
async def cq_show_settings(
    ...
    await _edit_settings_message(
        callback.message, settings_text, keyboard.as_markup(), localizer
    )
    try:
...
async def cq_set_parameter(
    ...
    if parameter == "temperature":
        ...
        default_temp_text = localizer.format_value(
            "settings-option-default",
            args={"value": f"{DEFAULT_GEMINI_TEMPERATURE:.1f}"},
        )
        ...
        default_tokens_text = localizer.format_value(
            "settings-option-default", args={"value": str(DEFAULT_GEMINI_MAX_TOKENS)}
        )
    ...
    await _edit_settings_message(
        callback.message, prompt_text, builder.as_markup(), localizer
    )
    try:
        await callback.answer()
    except Exception:
...
async def cq_save_value(callback: types.CallbackQuery, localizer: FluentLocalization):
    ...
    if parameter == "temperature":
        ...
        try:
            ...
            allowed_values_list = list(ALLOWED_TEMPERATURES.values()) + [
                DEFAULT_GEMINI_TEMPERATURE
            ]
            ...
            logger.warning(
                f"Incorrect value for temperature '{value_str}' from user_id={user_id}"
            )
        ...
        try:
            ...
            allowed_values_list = list(ALLOWED_MAX_TOKENS.values()) + [
                DEFAULT_GEMINI_MAX_TOKENS
            ]
            ...
            logger.warning(
                f"Incorrect value for max_tokens '{value_str}' from user_id={user_id}"
            )
            await callback.answer("Incorrect value!", show_alert=True)
            return
    else:
    ...
    if final_value is not None:
        try:
            ...
            if not setting_saved:
                logger.error(
                    f"Settings: save_user_setting returned False for user {user_id}, field {db_field_name}"
                )
                error_text, _ = format_error_message(DATABASE_SAVE_ERROR, localizer)
        except Exception as e_db:
            logger.exception(
                f"Settings: Error saving setting for user {user_id} to DB: {e_db}"
            )
            error_text, _ = format_error_message(DATABASE_SAVE_ERROR, localizer)
    ...
    if setting_saved:
        ...
        try:
            ...
            logger.warning(
                f"Settings: Could not answer callback with error for user {user_id}: {e_ans}"
            )
