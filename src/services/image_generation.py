...
if config and config.hf and config.hf.api_token:
    ...
    logger.warning(
        "Token for Hugging Face API not found. Image generation will not be available."
    )
...
async def generate_image_from_prompt(prompt: str) -> Tuple[Optional[bytes], str]:
    ...
    if hf_client is None or not config:
        logger.error(
            "Hugging Face client not configured for image generation or config not found."
        )
        return None, IMAGE_GEN_API_ERROR

    model_id = config.hf.image_gen_model_id
    logger.info(
        f"Requesting image generation with model '{model_id}'. Prompt: {prompt[:50]}..."
    )

    try:
        loop = asyncio.get_running_loop()
        image: Image.Image = await loop.run_in_executor(
            None, lambda: hf_client.text_to_image(prompt, model=model_id)
        )
        ...
        logger.info(
            f"Image generation completed with model '{model_id}'. Prompt: {prompt[:50]}..."
        )
        ...
        logger.error(
            f"HTTP error {status_code} from Hugging Face API ({model_id}): {error_message}. Details: {error_details}",
            exc_info=False,
        )
        if status_code == 429:
            ...
            "safety checker" in error_message.lower() or "nsfw" in error_message.lower()
        ):
        ...
        logger.warning(
            f"Timeout error during image generation with model ({model_id}): {e}",
            exc_info=False,
        )
        return None, IMAGE_GEN_TIMEOUT_ERROR
    except ConnectionError as e:
        logger.error(
            f"Connection error during image generation with model ({model_id}): {e}",
            exc_info=True,
        )
        return None, IMAGE_GEN_CONNECTION_ERROR
    except Exception as e:
        logger.error(
            f"Unexpected error while generating image with model ({model_id}): {e}",
            exc_info=True,
        )
